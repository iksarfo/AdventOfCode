...
def take_step (current, movement):
    direction = movement[0]
    distance = int(movement[1:])

    plot = { "R": (0,1), "U": (-1, 0), "L": (0, -1), "D": (1, 0) }
    moveY = plot[direction][0]
    moveX = plot[direction][1]
    move = (moveY, moveX)
...
def get_route (start, directions):
    current = start
    route = set(current)

    for move in directions:
        for step in take_step(current, move):
            current = step
            route.add(current)

    return route

def get_intersects (start, directions, previous):
    current = start
    intersects = []

    for path in directions:
        for step in take_step(current, path):
            current = step
...
def min_intersect (coordinates):
    return min(coordinates, key=lambda point: distance(point))

def test_case_1():
    route = get_route(start, ["R8","U5","L5","D3"])
    intersects = get_intersects(start, ["U7","R6","D4","L4"], route)
    closest = min_intersect(intersects)
    assert(6 == distance(closest))

def test_case_2():
    route = get_route(start, ["R75","D30","R83","U83","L12","D49","R71","U7","L72"])
    intersects = get_intersects(start, ["U62","R66","U55","R34","D71","R55","D58","R83"], route)
    closest = min_intersect(intersects)
    assert(159 == distance(closest))

def test_case_3():
    route = get_route(start, ["R98","U47","R26","D63","R33","U87","L62","D20","R33","U53","R51"])
    intersects = get_intersects(start, ["U98","R91","D20","R16","D67","R40","U7","R15","U6","R7"], route)
    closest = min_intersect(intersects)
    assert(135 == distance(closest))
